...
class LevelParser {
  ...
  createActors(stringsArray) {
    ...
    if (this.dict === undefined) {
      ...
      for (let y in stringsArray) {
        if ((stringsArray[y] === undefined) || (stringsArray[y] === '')) {
        } else {
          let symbolRow = stringsArray[y].split('');
          for (let x in symbolRow) {
            let result = this.actorFromSymbol(symbolRow[x]);
            if (result !== undefined) {
              resultArray.push(new result(new Vector(x, y)));
            }
          }
        }
      }
...
class Coin extends Actor {
  constructor(position) {
    ...
    this.pos = this.initialPos.plus(shift);
    this.pos.x = this.initialPos.x + 0.2;
    this.pos.y = this.initialPos.y + 0.1;
    this.springSpeed = 8;
  ...
  getNextPosition(time = 1) {
    ...
    let y = this.initialPos.y + springVector.y;
    return new Vector(x, y);
  }
  act(time) {
    this.pos = this.getNextPosition(time);
